...
for (const chart of charts) {
  ...
  const extractString = (regex) => {
    const match = content.match(regex);
    if (!match || !match[1]) return '';
    return match[1].replace(/['"`]/g, '').trim();
  };

  const heroEyebrow = extractString(/const heroEyebrow = ['"](.*?)['"];/);
  const heroTitle = extractString(/const heroTitle = ['"](.*?)['"];/);
  const heroDescriptionMatch = content.match(
    /const heroDescription =\s*(['"`])([\s\S]*?)\1;/
  );
  const heroDescription = heroDescriptionMatch
    ? heroDescriptionMatch[2].trim().replace(/\n/g, ' ').replace(/\s+/g, ' ')
    : '';

  const camelCase = chart
    .split('-')
  ...
  const obj = {
    [camelCase]: {
      ...
      whyItWorksTitle: extractString(/Why .*? Work/i) || 'Why It Works',
      ...
      useCase1: 'Compare product sales across categories or regions.',
      useCase2: 'Summarize survey responses by answer option.',
      useCase3: 'Show traffic, users, or conversions by channel.',
      useCase4:
      ...
      faq5Question: 'How is a chart different from a line chart?',
      faq5Answer:
        'A chart focuses on category comparison, while a line chart is usually better for showing continuous trends over time.',
      faq6Question: 'Why not just use a more complex spreadsheet chart?',
      faq6Answer:
      ...
      ctaTitle: 'Build a clean chart in minutes.',
      ctaDescription:
      ...
      ctaButton: 'Start With The Editor',
    },
  };

  fs.writeFileSync(
    path.join(chartsDir, chart, 'i18n', 'en.json'),
    JSON.stringify(obj, null, 2) + '\n'
  );
  fs.writeFileSync(
    path.join(chartsDir, chart, 'i18n', 'zh.json'),
    JSON.stringify(obj, null, 2) + '\n'
  );
  console.log(`Created i18n files for ${chart}`);
}
