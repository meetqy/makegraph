...
`相关矩阵`、`旭日图`、`乐队座位图` 和 `人口金字塔图` 页面也已接入：`https://makegraph.org/charts/correlation-matrix-chart`、`https://makegraph.org/charts/sunburst-chart`、`https://makegraph.org/charts/band-seating-chart` 与 `https://makegraph.org/charts/population-pyramid`。对应的 OG 预览图路径已在代码中预留，后续可再补充到 `public/charts/`。
