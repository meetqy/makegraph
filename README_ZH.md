...
MakeGraph 是 [makegraph.org](https://makegraph.org) 的代码仓库。它是一个适用于 Excel 和 CSV 数据的免费在线图表制作工具，基于 Next.js、TypeScript 和 React 构建。

![MakeGraph 预览图](public/og.png)

## 项目概览

MakeGraph 关注一条非常明确的链路：上传或粘贴电子表格数据，在大约 1 分钟内生成清晰图表，并尽量降低使用门槛。

当前仓库包含：

- 面向电子表格数据输入和实时预览的交互式图表编辑器
- 围绕柱状图、折线图、饼图等核心需求展开的图表页面与说明文案
- 面向 PNG、SVG 导出表达的产品和内容文案
- 与图表互相关联的博客、模板页
...
- 面向 Excel 和 CSV 工作流的数据上传或粘贴
- 全屏图表编辑器与实时预览
- 面向柱状图、折线图等核心类型的快速出图流程
- 图表内容导出为 PNG，并在产品定位文案中统一强调 SVG 导出
...
`置信区间面积图`、`相关矩阵`、`马里梅科图`、`旭日图`、`乐队座位图`、`帕累托图` 和 `人口金字塔图` 页面也已接入：`https://makegraph.org/charts/confidence-interval-area-chart`、`https://makegraph.org/charts/correlation-matrix-chart`、`https://makegraph.org/charts/marimekko-chart`、`https://makegraph.org/charts/sunburst-chart`、`https://makegraph.org/charts/band-seating-chart`、`https://makegraph.org/charts/pareto-chart` 与 `https://makegraph.org/charts/population-pyramid`。对应的 OG 预览图路径已在代码中预留，后续可再补充到 `public/charts/`。
