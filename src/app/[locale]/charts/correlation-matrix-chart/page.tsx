...
import { CorrelationChartMaker } from './_components/correlation-chart-maker';

const currentPath = '/charts/correlation-matrix-chart';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'CorrelationChart' });

  return {
    title: t('heroTitle').replace(/[.!。]+$/u, ''),
    description: t('heroDescription'),
    alternates: getMetadataAlternates(
      '/charts/correlation-matrix-chart',
      locale
    ),
    openGraph: {
      images: ['/charts/correlation-matrix-chart-og-image.png'],
    },
    twitter: {
      images: ['/charts/correlation-matrix-chart-og-image.png'],
    },
...
export default async function CorrelationChartPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
          ...
          <section className="py-16 first:pt-0">
            ...
            <div className="mt-12">
              <div className="flex w-full items-center justify-center overflow-hidden rounded-2xl border border-[#ebebeb] bg-[#fafafa]">
                <Image
                  src="/charts/correlation-matrix-chart-og-image.png"
                  alt="Correlation Matrix Example"
                  width={1200}
                  height={630}
