...
import { MarimekkoChartMaker } from './_components/marimekko-chart-maker';

const currentPath = '/charts/marimekko-chart';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'MarimekkoChart' });

  return {
    title: t('heroTitle').replace(/[.!。]+$/u, ''),
    description: t('heroDescription'),
    alternates: getMetadataAlternates('/charts/marimekko-chart', locale),
    openGraph: {
      images: ['/charts/marimekko-chart-og-image.png'],
    },
    twitter: {
      images: ['/charts/marimekko-chart-og-image.png'],
    },
...
export default async function MarimekkoChartPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
          ...
          <section className="py-16 first:pt-0">
            ...
            <div className="mt-12">
              <div className="flex w-full items-center justify-center overflow-hidden rounded-2xl border border-[#ebebeb] bg-[#fafafa]">
                <Image
                  src="/charts/marimekko-chart-og-image.png"
                  alt="Marimekko Chart Example"
                  width={1200}
                  height={630}
