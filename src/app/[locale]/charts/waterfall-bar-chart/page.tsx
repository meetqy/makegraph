...
import { WaterfallBarChartMaker } from './_components/waterfall-bar-chart-maker';

const currentPath = '/charts/waterfall-bar-chart';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'WaterfallBarChart' });

  return {
    title: generateChartTitle('Waterfall Bar Chart'),
    description: t('heroDescription'),
    alternates: getMetadataAlternates(currentPath, locale),
    openGraph: {
      images: ['/charts/waterfall-bar-chart-og-image.png'],
    },
    twitter: {
      images: ['/charts/waterfall-bar-chart-og-image.png'],
    },
...
export default async function WaterfallBarChartPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
          ...
          <section className="py-16 first:pt-0">
            ...
            <div className="mt-12">
              <div className="flex w-full items-center justify-center overflow-hidden rounded-2xl border border-[#ebebeb] bg-[#fafafa]">
                <Image
                  src="/charts/waterfall-bar-chart-og-image.png"
                  alt="Waterfall Bar Chart Example"
                  width={1200}
                  height={630}
