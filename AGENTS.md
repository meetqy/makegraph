...
- 统计图预览区域默认直接展示图表本体，不额外增加外层 border/card 包裹，也不在图内额外渲染标题和描述；如确有需要，必须由用户明确提出。
...
- 后续所有页面文案、chart 文案、FAQ、blog、README、metadata 以及其他对外内容，默认都必须兼顾 SEO / GEO 友好，不能只满足页面展示而忽略搜索与生成式检索场景。
- SEO / GEO 文案必须优先保证主题明确、实体清晰、用户问题导向和自然语言表达；标题、描述、正文、FAQ、CTA 需要准确出现当前页面的核心主题词，禁止出现目录是 A、文案却在泛讲 B 的语义错位。
- 所有对外内容默认必须符合 E-E-A-T：内容要体现真实使用场景、专业判断、边界条件和清晰结论；避免空泛营销词、模板化套话、关键词堆砌和缺乏主体词的模糊表达。
- 图表页面文案默认需要回答用户真实会搜索或会提问的问题，例如“这是什么图”“适合什么数据”“什么时候该用”“什么时候不该用”“和其他图表有什么区别”，以增强 SEO、GEO 和内容可信度。
